...
namespace Movie_Store_App_Services
{
    public class MovieService
    {
        ...
        public Movie AddRentedMovies(int chosenMovie, User user)
        {
            ...
            newarray[newarray.Length - 1] = Movies[chosenMovie - 1];
            Movies[chosenMovie - 1].IsRented = true;
        ...
        public void RentLogic(User user)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine($"Please choose a film. Enter a number between 1 and {Movies.Length}");
                Console.WriteLine("The ones with an asterisk are currently not available!");
                
                int chosenMovie = helperService.ValidatePositiveNumber(Console.ReadLine(), Movies.Length);
                if (chosenMovie >= 1 && chosenMovie <= Movies.Length)
                {
                    if (Movies[chosenMovie - 1].IsRented == false)
                    {
                        if (helperService.ContinueRenting() == true)
                        {
                    ...
                    {
                        Console.Clear();
                        Console.WriteLine("The movie is already rented!");
                        Console.ReadLine();
                        Console.Clear();
                        if (helperService.ContinueRenting() == false)
                        {
                            Console.Clear();
                            Console.WriteLine("These are your rented movies:");
                            PrintRentedMovies(user);
